...
export const HopitalModel = {
  ...
  create: (nom, adresse, telephone, callback) => {
    const query = 'INSERT INTO hopital (nom, adresse, telephone) VALUES (?, ?, ?)';
  ...
  getById: (id, callback) => {
    const query = 'SELECT * FROM hopital WHERE id = ?';
  ...
  update: (id, nom, adresse, telephone, callback) => {
    const query = 'UPDATE hopital SET nom = ?, adresse = ?, telephone = ? WHERE id = ?';
  ...
  delete: (id, callback) => {
    const query = 'DELETE FROM hopital WHERE id = ?';
    db.query(query, [id], (err, results) => {
      if (err) {
